...
# Bot owner ID
OWNER_ID = 911655070817456139  # KENDİ ID'NİZİ BURAYA GİRİN
...
async def userinfo(interaction: discord.Interaction, kullanici: discord.Member = None):
    ...
    embed.add_field(name="🌐 Aktiflik Durumu", value=durum_emoji.get(kullanici.status, "⚫ Bilinmiyor"), inline=True)

    # Sunucu İstatistikleri
    embed.add_field(name="📅 Sunucuya Katılma", value=f"{kullanici.joined_at.strftime('%d %B %Y %H:%M:%S')}\n`{sunucu_yasi} gün önce`", inline=True)
    embed.add_field(name="💬 Mesaj Sayısı", value="```Hesaplanıyor...```", inline=True)
    embed.add_field(name="🔊 Ses Süresi", value="```00:00:00```", inline=True)

    # Yetkili İstatistikleri
    warn_count = len([w for w in warnings.get(kullanici.id, []) if 'Ban' in w.get('sebep', '')])
    mute_count = len([w for w in warnings.get(kullanici.id, []) if 'Mute' in w.get('sebep', '')])
    embed.add_field(name="👑 Ban Sayısı", value=f"```{warn_count}```", inline=True)
    embed.add_field(name="🔇 Mute Sayısı", value=f"```{mute_count}```", inline=True)
    embed.add_field(name="🔒 Jail Sayısı", value=f"```0```", inline=True)
    ...
    if banner_url:
        ...
        embed.add_field(name="🖼️ Banner", value="❌ Kullanıcının banner'ı yok.", inline=False)
...
# ==================== YENİ /roller KOMUTU (Sayfalandırılmış) ====================
@bot.tree.command(name="roller", description="Sunucudaki rolleri sayfa sayfa gösterir")
async def roller(interaction: discord.Interaction):
    ...
    class RolePaginationView(discord.ui.View):
        ...
        def update_buttons(self):
            ...
            filter_labels = {
                "all": "Tüm Roller",
                "top": "Yüksekten Düşüğe",
                "bottom": "Düşükten Yükseğe",
                "members": "Çok üyeden Az üyeye",
                "few_members": "Az üyeden Çok üyeye"
            }
            self.children[1].label = filter_labels.get(self.filter_type, "Tüm Roller")
        ...
        @discord.ui.select(
            ...
            options=[
                discord.SelectOption(label="Tüm Roller", value="all"),
                discord.SelectOption(label="Yüksekten Düşüğe", value="top"),
                discord.SelectOption(label="Düşükten Yükseğe", value="bottom"),
                discord.SelectOption(label="Çok üyeden Az üyeye", value="members"),
                discord.SelectOption(label="Az üyeden Çok üyeye", value="few_members"),
            ]
        )
        async def filter_select(self, interaction: discord.Interaction, select):
            self.filter_type = select.values[0]
            if self.filter_type == "top":
                self.roles_list = sorted(guild.roles[1:], key=lambda r: r.position, reverse=True)
            elif self.filter_type == "bottom":
                self.roles_list = sorted(guild.roles[1:], key=lambda r: r.position)
            elif self.filter_type == "members":
                self.roles_list = sorted(guild.roles[1:], key=lambda r: len(r.members), reverse=True)
            elif self.filter_type == "few_members":
                self.roles_list = sorted(guild.roles[1:], key=lambda r: len(r.members))
            else:
                self.roles_list = sorted(guild.roles[1:], key=lambda r: r.position, reverse=True)

            self.current_page = 0
            self.total_pages = (len(self.roles_list) + self.page_size - 1) // self.page_size
            self.update_buttons()
        ...
        async def update_message(self, interaction: discord.Interaction):
            ...
            embed = discord.Embed(
                title=f"📋 {guild.name} Sunucusunun Rolleri [{start+1}-{end} / {len(self.roles_list)}]",
                color=0x5865F2,
                timestamp=datetime.now(timezone.utc)
            )
            embed.set_thumbnail(url=guild.icon.url if guild.icon else None)
            desc = "\n".join(f"{role.mention} (`{len(role.members)}`)" for role in page_roles) or "`Rol bulunamadı.`"
            embed.description = desc
            embed.set_footer(text=f"Filtre: {self.filter_type} | İsteyen: {interaction.user.name}")
            self.update_buttons()
            await interaction.response.edit_message(embed=embed, view=self)

    view = RolePaginationView(roles)
    start = 0
    end = min(15, len(roles))
    first_page = roles[start:end]
    embed = discord.Embed(
        title=f"📋 {guild.name} Sunucusunun Rolleri [{start+1}-{end} / {len(roles)}]",
        color=0x5865F2,
        timestamp=datetime.now(timezone.utc)
    )
    embed.set_thumbnail(url=guild.icon.url if guild.icon else None)
    desc = "\n".join(f"{role.mention} (`{len(role.members)}`)" for role in first_page) or "`Rol bulunamadı.`"
    embed.description = desc
    embed.set_footer(text=f"Filtre: Tüm Roller | İsteyen: {interaction.user.name}")
...
import os
# ...
TOKEN = os.getenv('DISCORD_BOT_TOKEN') # Örneğin, Railway'de KEY bu isim olmalı
# ...
bot.run(TOKEN)
